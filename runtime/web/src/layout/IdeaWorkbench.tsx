...
export function IdeaWorkbench(props: Props) {
  ...
  useEffect(() => {
    if (previousView.current !== view) {
      if (view === "chat") chatRef.current?.focus();
  ...
  return (
    <div className="idea-workbench" data-onboarding="shell" data-idea-view={view} data-idea-chrome={chrome ? "true" : undefined}>
      ...
      {view !== "chat" ? <header className="idea-view-heading">
        <button type="button" className="idea-icon-button" aria-label={t("idea.backToChat")} title={t("idea.backToChat")} onClick={backToChat}><Icon kind="back" /></button>
        <div className="idea-view-heading-title">
          <h1 ref={headingRef} tabIndex={-1}>{view === "settings" ? t("idea.settings") : t("idea.history")}</h1>
      ...
      <section className="idea-chat" hidden={view !== "chat"} aria-label={t("idea.chat")}>
        <main ref={chatRef} tabIndex={-1} className="idea-conversation">{props.chat}{props.drawer}</main>
        <footer className="idea-composer">
          {chrome || props.composerAction ? <div className="idea-file-context-actions">
            {chrome ? (
