...
export function FileTree({
  ...
  const shouldShowInstallButton = !relayNoRelayer && !isNativeApp && !isKnownInstalled && !(isAndroidChrome && !deferredInstallPrompt);
  const shouldShowInstallHelp = !relayNoRelayer && !isNativeApp && (!!installHelp) && (isKnownInstalled || isIOS || isMacSafari || isDesktopChromium || deferredInstallPrompt !== null || (isAndroidChrome && !deferredInstallPrompt));
      ...
      <div style={{ position: "relative", height: "36px", padding: "0 3px", borderBottom: "1px solid var(--border-color)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "var(--mindfs-topbar-bg, transparent)", boxSizing: "border-box", flexShrink: 0, gap: 0, overflow: "visible" }}>
        ...
        <div ref={menuRef} style={{ position: "relative", flexShrink: 0 }}>
          ...
          {isMenuOpen ? (
            ...
            >
                {(onOpenProjectAdd || onCreateRootStart) && <button
                  type="button"
                  onClick={() => {
                    if (onOpenProjectAdd) {
                      onOpenProjectAdd();
