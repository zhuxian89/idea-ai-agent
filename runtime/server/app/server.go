...
type StartOptions struct {
	// LocalOnly is used by the IntelliJ host. Agent networking remains available.
	LocalOnly       bool
	ProjectRoot     string
	AccessToken     string
...
func Start(ctx context.Context, addr string, opts StartOptions) error {
	if opts.LocalOnly {
		if err := validateLocalOptions(addr, opts); err != nil {
			return err
		}
		opts.NoRelayer = true
		opts.WebPushEnabled = false
	}
	registry, err := fs.NewDefaultRegistry()
	if err != nil {
		return err
	}
	// The IDE host selects the only managed root. Discard stale registrations
	// from previous runtime versions before starting schedulers or Agent probes.
	if !opts.LocalOnly {
	...
	rootID := ""
	if opts.ProjectRoot != "" {
		root, err := registry.Upsert(opts.ProjectRoot)
		if err != nil {
			return err
		}
		rootID = root.ID
	...
	var handler http.Handler = api.LoggingMiddleware(api.CORSMiddleware(mux))
	if opts.LocalOnly {
		handler = localAccessMiddleware(opts.AccessToken, rootID, api.LoggingMiddleware(mux))
	}
	...
	if opts.OnReady != nil {
		if err := opts.OnReady(listener.Addr().String(), rootID); err != nil {
			return err
		}
	}
