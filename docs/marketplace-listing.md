...
This file contains the values to use for the JetBrains Marketplace listing of Local AI Agent.
...
JetBrains reads this description from an uploaded plugin ZIP. Before uploading 0.1.23, the approved public page can be updated without advertising unreleased features: in **General Information → Description**, choose **use the UI description until next update** and paste the already-released `docs/marketplace-description-0.1.22.html`. The 0.1.23 upload will then replace it with the canonical bilingual `plugin.xml` description. `docs/marketplace-description-0.1.23.html` is the post-approval copy.

## Getting Started

Paste `docs/marketplace-getting-started-0.1.23.html` into the Marketplace **Getting Started** field. It starts with the zero-configuration native Agent flow, then explains optional voice setup and recording. English is first and Simplified Chinese follows. Update it together with the 0.1.23 upload so the public page does not advertise unavailable features.

## Version 0.1.23 change notes

The canonical change notes are in `src/main/resources/META-INF/plugin.xml` and are included in the built ZIP. Version 0.1.23 focuses on reliable turn-change tracking, inline turn diffs, and IDEA native side-by-side diff integration.
...
GitHub Releases provide separate packages for Windows x64, Windows ARM64, macOS Apple Silicon, and macOS Intel. Upload `idea-ai-agent-0.1.23.zip` to Marketplace; it is the single universal plugin package and contains all four runtime executables. IDEA selects the matching executable when the plugin starts. IDEA 2024.x does not support Marketplace native variants.
...
- Confirm the built `plugin.xml` contains the English-first bilingual description, 0.1.23 change notes, `since-build="241"`, and no `until-build` attribute.
